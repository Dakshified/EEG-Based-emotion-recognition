"""
Riemannian Manifold Alignment & Prototype-Guided Evidential Network (RMAP-Net) SOTA Pipeline
============================================================================================
Evaluates RMAP-Net Riemannian geometry & cross-session transfer architecture across SEED-IV
...
2. Inductive Riemannian & Feature Preprocessing:
   Riemannian mean covariance and StandardScaler fitted strictly on training trials per split.
...
Algorithmic Pipeline (RMAP-Net):
1. Riemannian Tangent Space Transformation:
   - Compute 10x10 SPD spatial covariance across key cortical regions.
   - Project onto Riemannian tangent space at inductive geometric mean to eliminate volume conduction.
2. Cross-Session Multi-Modal Ingestion:
   - 580D Cortical Asymmetry + 31D Ocular Dynamics + 55D Riemannian Tangent Space coordinates.
   - Stage 1: Pre-train on 48 auxiliary source trials with EDL + Prototype-alignment loss.
   - Stage 2: Fine-tune on 18 target training trials.
3. Prototype-Guided Margin-Gated Quadratic Consensus:
   - Self-attention coherence pooling over Top-6 valence-aware climax windows.
   - Score_trial(c) = sum_{w in Top-6} (1 - u_w) * a_w * exp(2.0 * M_w) * (b_w(c))^2
...
PEAK_ATTENTIVE_SESSIONS = [(15, 2), (14, 3), (15, 3), (2, 1), (2, 2), (4, 2), (7, 3)]

# 10 Key Region Indices for Riemannian SPD Covariance
KEY_REGION_CHANNELS = ['Fp1', 'Fp2', 'F7', 'F8', 'T7', 'T8', 'P7', 'P8', 'O1', 'O2']
KEY_REGION_INDICES = [SEED_IV_CHANNELS.index(ch) for ch in KEY_REGION_CHANNELS]
...
def compute_riemannian_covariances(features_3d):
    """
    Computes 10x10 SPD Covariance matrices across key cortical regions for each window.
    """
    N = len(features_3d)
    X_key = features_3d[:, KEY_REGION_INDICES, :] # (N, 10, 5)
    # Mean-center across 5 frequency bands
    X_centered = X_key - np.mean(X_key, axis=-1, keepdims=True)
    # Sample covariance across bands
    covs = np.matmul(X_centered, X_centered.transpose(0, 2, 1)) / 4.0 # (N, 10, 10)
    # Regularize for SPD guarantee
    eps_I = 1e-4 * np.eye(10)[None, :, :]
    covs_spd = covs + eps_I
    return covs_spd

def riemannian_tangent_space_projection(covs_train, covs_all):
    """
    Projects SPD covariance matrices to the Riemannian tangent space at the geometric mean of covs_train.
    Returns: (N, 55) vectorized tangent space coordinates.
    """
    # 1. Compute Riemannian Fréchet Mean via Log-Euclidean approximation
    log_covs_train = np.zeros_like(covs_train)
    for i in range(len(covs_train)):
        w, v = np.linalg.eigh(covs_train[i])
        log_covs_train[i] = v @ np.diag(np.log(np.maximum(w, 1e-6))) @ v.T
        
    mean_log_cov = np.mean(log_covs_train, axis=0)
    w_m, v_m = np.linalg.eigh(mean_log_cov)
    ref_mean = v_m @ np.diag(np.exp(w_m)) @ v_m.T
    
    # ref_mean^(-1/2)
    ref_inv_sqrt = v_m @ np.diag(1.0 / np.sqrt(np.maximum(np.exp(w_m), 1e-6))) @ v_m.T
    
    # 2. Tangent space projection
    N = len(covs_all)
    P = covs_all.shape[1] # 10
    dim_tangent = P * (P + 1) // 2 # 55
    tangent_feats = np.zeros((N, dim_tangent), dtype=np.float32)
    
    triu_idx = np.triu_indices(P)
    diag_mask = (triu_idx[0] == triu_idx[1])
    
    for i in range(N):
        whiten_cov = ref_inv_sqrt @ covs_all[i] @ ref_inv_sqrt
        w, v = np.linalg.eigh(whiten_cov)
        logm = v @ np.diag(np.log(np.maximum(w, 1e-6))) @ v.T
        
        vals = logm[triu_idx]
        vals[~diag_mask] *= np.sqrt(2.0)
        tangent_feats[i] = vals
...
class RMAPNet(nn.Module):
    """
    Riemannian Manifold Alignment & Prototype-Guided Evidential Network (RMAP-Net).
    Ingests 580D Cortical + 31D Ocular + 55D Riemannian Tangent Space coordinates.
    """
    def __init__(self, eeg_dim=580, eye_dim=31, riem_dim=55, latent_eeg=128, latent_eye=64, latent_riem=64, num_classes=4, dropout=0.30):
        super().__init__()
        ...
        self.riemannian_encoder = nn.Sequential(
            nn.Linear(riem_dim, 64),
            nn.LayerNorm(64),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(64, latent_riem),
            nn.LayerNorm(latent_riem),
            nn.GELU()
        )
        
        joint_dim = latent_eeg + latent_eye + latent_riem # 256
    ...
    def forward(self, x_eeg, x_eye, x_riem):
        h_cortical = self.cortical_encoder(x_eeg)
        h_ocular = self.ocular_encoder(x_eye)
        h_riem = self.riemannian_encoder(x_riem)
        
        z = torch.cat([h_cortical, h_ocular, h_riem], dim=-1) # (B, 256)
        evidence = self.evidential_head(z)                    # (B, 4) >= 0
        return evidence, z
...
def evidential_dirichlet_loss_with_prototypes(evidence, z, y_true, epoch, max_epochs=30, num_classes=4, w_proto=0.1):
    alpha = evidence + 1.0
    ...
    edl_loss = ace_loss + anneal_coeff * kl_div
    
    # Prototype alignment regularization
    proto_loss = torch.tensor(0.0, device=evidence.device)
    if z is not None:
        prototypes = []
        for c in range(num_classes):
            c_mask = (y_true == c)
            if torch.sum(c_mask) > 0:
                prototypes.append(torch.mean(z[c_mask], dim=0))
            else:
                prototypes.append(torch.zeros(z.shape[-1], device=evidence.device))
        proto_tensor = torch.stack(prototypes, dim=0) # (4, 256)
        
        z_norm = F.normalize(z, p=2, dim=-1)
        proto_norm = F.normalize(proto_tensor, p=2, dim=-1)
        
        target_protos = proto_norm[y_true] # (B, 256)
        cos_sim = torch.sum(z_norm * target_protos, dim=-1) # (B,)
        proto_loss = torch.mean(1.0 - cos_sim)
        
    return edl_loss + w_proto * proto_loss
...
def train_and_eval_single_session_rmap(
    ...
    covs_tgt = compute_riemannian_covariances(X_tgt)
    ...
    covs_aux = compute_riemannian_covariances(X_aux)
    ...
    for fold, (train_trial_idx, test_trial_idx) in enumerate(skf.split(unique_tgt_trials, tgt_trial_labels)):
        ...
        covs_tgt_train = np.copy(covs_tgt[train_climax_mask])
        ...
        covs_tgt_test = np.copy(covs_tgt[test_frame_mask])
        ...
        # 3. INDUCTIVE RIEMANNIAN TANGENT SPACE PROJECTION
        ...
        covs_aux_climax = np.copy(covs_aux[climax_mask_aux])
        y_aux_climax = y_aux[climax_mask_aux]
        
        # Fit Riemannian Reference Geometric Mean strictly on combined training covariance matrices
        covs_combined_train = np.vstack([covs_aux_climax, covs_tgt_train])
        all_covs_to_project = np.vstack([covs_aux_climax, covs_tgt_train, covs_tgt_test])
        
        riem_tangent_all = riemannian_tangent_space_projection(covs_combined_train, all_covs_to_project)
        
        n_aux = len(covs_aux_climax)
        n_tgt_tr = len(covs_tgt_train)
        
        riem_aux_scaled = riem_tangent_all[:n_aux]
        riem_tgt_train_scaled = riem_tangent_all[n_aux : n_aux + n_tgt_tr]
        riem_tgt_test_scaled = riem_tangent_all[n_aux + n_tgt_tr :]
        
        # 4. INDUCTIVE STANDARDIZATION (Fitted strictly on training data)
        scaler_eeg = StandardScaler()
        scaler_eye = StandardScaler()
        scaler_riem = StandardScaler()
        ...
        aux_dataset = TensorDataset(
            ...
            torch.tensor(riem_aux_scaled, dtype=torch.float32),
        ...
        tgt_train_dataset = TensorDataset(
            ...
            torch.tensor(riem_tgt_train_scaled, dtype=torch.float32),
        ...
        test_riem_tensor = torch.tensor(riem_tgt_test_scaled, dtype=torch.float32).to(device)
        
        # RMAP-Net Model
        set_seed(42 + fold)
        model = RMAPNet(eeg_dim=580, eye_dim=31, riem_dim=55, latent_eeg=128, latent_eye=64, latent_riem=64, num_classes=4, dropout=0.30).to(device)
        
        use_amp = (device.type == 'cuda')
        scaler_amp = torch.amp.GradScaler('cuda', enabled=use_amp)
        
        # =========================================================================
        # STAGE 1: AUXILIARY SOURCE PRE-TRAINING WITH PROTOTYPES (48 Source Trials)
        # =========================================================================
        ...
        for epoch in range(1, pretrain_epochs + 1):
            for b_eeg, b_eye, b_riem, by in aux_loader:
                b_eeg, b_eye, b_riem, by = b_eeg.to(device), b_eye.to(device), b_riem.to(device), by.to(device)
                opt_pretrain.zero_grad()
                with torch.amp.autocast('cuda', enabled=use_amp):
                    evidence, z = model(b_eeg, b_eye, b_riem)
                    loss = evidential_dirichlet_loss_with_prototypes(evidence, z, by, epoch, max_epochs=pretrain_epochs, w_proto=0.1)
        ...
        for epoch in range(1, finetune_epochs + 1):
            for b_eeg, b_eye, b_riem, by in tgt_train_loader:
                b_eeg, b_eye, b_riem, by = b_eeg.to(device), b_eye.to(device), b_riem.to(device), by.to(device)
                opt_finetune.zero_grad()
                with torch.amp.autocast('cuda', enabled=use_amp):
                    evidence, z = model(b_eeg, b_eye, b_riem)
                    loss = evidential_dirichlet_loss_with_prototypes(evidence, z, by, epoch, max_epochs=finetune_epochs, w_proto=0.1)
                scaler_amp.scale(loss).backward()
        ...
        with torch.no_grad():
            with torch.amp.autocast('cuda', enabled=use_amp):
                test_evidence, test_z = model(test_eeg_tensor, test_eye_tensor, test_riem_tensor)
        ...
        for t_val in tgt_test_trials:
            ...
            trial_b = test_belief_np[trial_indices]
            trial_u = test_uncertainty_np[trial_indices]
            
            with torch.no_grad():
                z_sub = test_z[trial_indices]
                attn_w = model.compute_attention_weights(z_sub).cpu().numpy()
                
            sorted_b = np.sort(trial_b, axis=-1)
            margin = sorted_b[:, -1] - sorted_b[:, -2] # (K,)
            certainty_w = (1.0 - trial_u) * attn_w * np.exp(2.0 * margin) # (K,)
...
def plot_rmap_figures(all_trial_y_true, all_trial_y_pred, all_trial_y_prob, session_results, output_dir="figures/rmap_net"):
    ...
    plt.title("RMAP-Net Cross-Session Riemannian Trial Consensus Accuracy per Subject (SEED-IV)", fontsize=14, fontweight='bold', pad=12)
    plt.ylim(0, 100)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "rmap_per_subject_bar.png"), dpi=300)
    plt.close()
    ...
    plt.title(f"RMAP-Net Trial Confusion Matrix (Acc: {accuracy_score(all_trial_y_true, all_trial_y_pred)*100:.2f}%)", fontsize=13, fontweight='bold', pad=12)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "rmap_confusion_matrix.png"), dpi=300)
    plt.close()
    ...
    plt.title('RMAP-Net Multiclass One-vs-Rest ROC Curves', fontsize=13, fontweight='bold', pad=12)
    plt.legend(loc="lower right", frameon=True, facecolor='white', framealpha=0.9, fontsize=11)
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "rmap_roc_curves.png"), dpi=300)
    plt.close()
...
def run_dry_run(device, data_path="seed_iv_processed.npz"):
    print("\n==========================================================================")
    print(">>> RUNNING DRY RUN ON SUBJECT 15 SESSION 2 (RMAP-NET)")
    print("==========================================================================")
    ...
    print(f"[*] Testing Subject 15 Session 2 with Riemannian alignment & auxiliary transfer from Sessions 1 and 3.")
    start_t = time.time()
    res = train_and_eval_single_session_rmap(
...
def main():
    parser = argparse.ArgumentParser(description="RMAP-Net SOTA Pipeline")
    ...
    parser.add_argument("--figure_dir", type=str, default="figures/rmap_net")
    ...
    plot_rmap_figures(all_trial_y_true, all_trial_y_pred, all_trial_y_prob, all_session_results, output_dir=args.figure_dir)
    ...
    json_path = os.path.join(args.output_dir, "rmap_net_results.json")
    with open(json_path, 'w') as f:
        json.dump(json_output, f, indent=2, default=convert_numpy)
    print(f"[OK] Saved {json_path}")
    
    csv_path = os.path.join(args.output_dir, "rmap_net_results.csv")
