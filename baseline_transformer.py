...
# =========================================================================
# PLAIN TRANSFORMER ARCHITECTURE (71,684 Trainable Parameters)
# =========================================================================

class PlainTransformer(nn.Module):
    def __init__(self, n_channels=62, in_features=5, d_model=64, nhead=4, num_layers=2, dim_feedforward=128, dropout=0.1, n_classes=4):
        super().__init__()
        self.input_proj = nn.Linear(in_features, d_model)
        self.pos_embedding = nn.Parameter(torch.randn(1, n_channels, d_model) * 0.02)
    ...
    def forward(self, x):
        ...
        h = self.norm(h)
        h = torch.mean(h, dim=1) # Global average pooling over 62 channel tokens
...
def train_and_evaluate_fold(X_train_raw, y_train, X_val_raw, y_val, X_test_raw, y_test, device, max_epochs=100, patience=15, batch_size=128):
    """Trains Plain Transformer with early stopping on validation accuracy and evaluates on test set."""
    ...
    model = PlainTransformer(n_channels=62, in_features=5, d_model=64, nhead=4, num_layers=2, dim_feedforward=128, dropout=0.1, n_classes=4).to(device)
...
def run_subject_dependent(features, labels, subject_ids, session_nums, trial_ids, device, figures_dir):
    ...
    print("=" * 80, flush=True)
    print("RUNNING PLAIN TRANSFORMER: Subject-Dependent Protocol (Pooled, 5-Fold Trial-Grouped CV)", flush=True)
    print("=" * 80, flush=True)
    ...
    for fold, (train_trial_idx, test_trial_idx) in enumerate(kf_outer.split(np.arange(num_trials))):
        ...
        res = train_and_evaluate_fold(X_train, y_train, X_val, y_val, X_test, y_test, device)
    ...
    prefix = "transformer_subject_dependent"
    ...
    np.savez_compressed(
        "oof_preds_transformer_subject_dependent.npz",
        y_true=y_true_all,
        y_pred=y_pred_all,
        y_prob=y_prob_all
    )
...
def run_cross_subject(features, labels, subject_ids, device, figures_dir):
    ...
    print("=" * 80, flush=True)
    print("RUNNING PLAIN TRANSFORMER: Cross-Subject Protocol (Leave-3-Subjects-Out)", flush=True)
    print("=" * 80, flush=True)
    ...
    prefix = "transformer_cross_subject"
    save_evaluation_plots(y_true_all, y_pred_all, y_prob_all, prefix, figures_dir)
    
    # Save out-of-fold sample predictions for verified downstream statistical testing
    np.savez_compressed(
        "oof_preds_transformer_cross_subject.npz",
        y_true=y_true_all,
        y_pred=y_pred_all,
        y_prob=y_prob_all
    )
...
def main():
    ...
    print("=" * 80, flush=True)
    print("PLAIN TRANSFORMER BASELINE (GPU)", flush=True)
    ...
    json_path = "transformer_baseline_results.json"
    with open(json_path, "w") as f:
        json.dump(all_results, f, indent=4)
    print(f"Saved structured JSON results to '{json_path}'", flush=True)
    
    csv_path = "transformer_baseline_results.csv"
    ...
    print("\n" + "=" * 80, flush=True)
    print("FINAL SUMMARY: PLAIN TRANSFORMER RESULTS", flush=True)
    print("=" * 80, flush=True)
    ...
    print("=" * 80, flush=True)
    print(f"Total Plain Transformer baseline pipeline runtime: {(time.time() - t_start)/60:.2f} minutes.", flush=True)
