"""
Peak-Decisive Super-Evidential Consensus Network (PSEC-Net) SOTA Pipeline
==========================================================================
Evaluates PSEC-Net multimodal cortical-ocular alignment architecture across SEED-IV
...
2. Adaptive Climax Dynamic Gating:
   - Drop first 3s (onset) and last 2s (offset) per trial.
   - Retain windows satisfying E_w >= mu_E + 0.25 * sigma_E (clamped to [4, 0.70 * L] windows).
3. Super-Evidential Belief Margin Accumulation:
   - Belief masses: b_w(c) = e_w(c) / S_w.
   - Belief margin: M_w = top1(b_w) - top2(b_w).
   - Super-evidential weight: W_w = (1.0 - u_w) * exp(2.0 * M_w).
   - Cold Likelihood with Bounded Outlier Clamping (tau = 0.5, clamp min=-1.8, max=0.0):
     Score_trial(c) = sum_{w in Adaptive_Climax} W_w * clamp(log(P_cold(c | x_w)), min=-1.8, max=0.0)
...
class PSECNet(nn.Module):
    """
    Peak-Decisive Super-Evidential Consensus Network (PSEC-Net).
    """
    def __init__(self, eeg_dim=580, eye_dim=31, latent_dim=128, num_classes=4):
        ...
        # Ocular Stream Encoder (LayerNorm for robust batch invariance)
...
def train_and_eval_single_session_psec(
    sub_id, sess_id, X_sess_3d, y_sess, trial_ids_sess, device, epochs=30, batch_size=32, n_splits=4, tau=0.5, clip_min=-1.8, beta=2.0
):
    unique_trials = []
    trial_labels = []
    ...
    for fold, (train_trial_idx, test_trial_idx) in enumerate(skf.split(unique_trials, trial_labels)):
        ...
        # PSEC-Net Model
        set_seed(42 + fold)
        model = PSECNet(eeg_dim=580, eye_dim=31, latent_dim=128, num_classes=4).to(device)
        ...
        with torch.no_grad():
            with torch.amp.autocast('cuda', enabled=use_amp):
                ...
                # Belief masses & top-1 vs top-2 belief margin
                test_belief = test_evidence / test_S
                sorted_belief, _ = torch.sort(test_belief, dim=-1, descending=True)
                test_margin = sorted_belief[:, 0:1] - sorted_belief[:, 1:2]
                
                # Super-Evidential Weight: W_w = (1 - u_w) * exp(beta * M_w)
                super_weight_tensor = (1.0 - test_uncertainty_tensor) * torch.exp(beta * test_margin)
                ...
                super_weight_np = super_weight_tensor.cpu().numpy().squeeze(-1)
        ...
        # Super-Evidential Belief Margin Accumulation on Climax
        for t_val in test_trials_fold:
            ...
            if np.sum(k_climax) > 0:
                ...
                trial_w = super_weight_np[k_climax]
            else:
                trial_p = test_probs[k_mask]
                trial_w = super_weight_np[k_mask]
                
            # Score_trial(c) = sum_{w in Adaptive_Climax} W_w * clamp(ln(P_cold(c)), min=-1.8, max=0.0)
            log_p = np.log(np.maximum(trial_p, 1e-7))
            log_p_clipped = np.clip(log_p, clip_min, 0.0)
            weighted_log_scores = np.sum(trial_w[:, None] * log_p_clipped, axis=0) # (4,)
...
def run_psec_benchmark(args):
    print("=" * 80, flush=True)
    print("PEAK-DECISIVE SUPER-EVIDENTIAL CONSENSUS NETWORK (PSEC-NET) SOTA BENCHMARK", flush=True)
    print("=" * 80, flush=True)
    ...
    if args.dry_run:
        print("\n" + "=" * 60, flush=True)
        print("EXECUTING AUTOMATED DRY RUN: Subject 02 Session 1", flush=True)
        print("=" * 60, flush=True)
        
        sub_mask = (subject_ids_all == 2) & (session_nums_all == 1)
        X_sub = features_all[sub_mask]
        y_sub = labels_all[sub_mask]
        t_sub = trial_ids_all[sub_mask]
        
        t0 = time.time()
        dry_res = train_and_eval_single_session_psec(
            2, 1, X_sub, y_sub, t_sub, device, epochs=args.epochs, batch_size=32, n_splits=4, tau=0.5, clip_min=-1.8, beta=2.0
        )
        t_elapsed = time.time() - t0
        
        print(f"\n[DRY RUN VERIFIED] Sub 02 Sess 1 ({dry_res['num_trials']} trials, {dry_res['num_frames']} frames in {t_elapsed:.1f}s):", flush=True)
        print(f"  * Frame Accuracy: {dry_res['frame_accuracy']*100:.2f}% (F1: {dry_res['frame_f1']:.4f}, AUC: {dry_res['frame_auc']:.4f})", flush=True)
        print(f"  * Super-Evidential Trial Accuracy: {dry_res['trial_accuracy']*100:.2f}% (F1: {dry_res['trial_f1']:.4f}, AUC: {dry_res['trial_auc']:.4f})", flush=True)
        ...
        print(f"  * Super-Evidential Weight Scaling: W_w = (1 - u_w) * exp(2.0 * M_w) | Outlier Clamping: Active [-1.8, 0.0]", flush=True)
        print("=" * 60, flush=True)
        return
    ...
    print("\nStarting PSEC-Net Benchmark across all 45 Sessions...", flush=True)
    print("-" * 95, flush=True)
    ...
    for sub_id in all_subjects:
        for sess_id in [1, 2, 3]:
            ...
            res = train_and_eval_single_session_psec(
                sub_id, sess_id, X_sub, y_sub, t_sub, device, epochs=args.epochs, batch_size=32, n_splits=4, tau=0.5, clip_min=-1.8, beta=2.0
            )
    ...
    print("\n" + "=" * 80, flush=True)
    print("FINAL BENCHMARK SUMMARY: PEAK-DECISIVE SUPER-EVIDENTIAL CONSENSUS (PSEC-NET)", flush=True)
    print("=" * 80, flush=True)
    ...
    out_json = 'psec_net_results.json'
    out_csv = 'psec_net_results.csv'
    ...
    ax.set_ylim(0, 105)
    ax.set_ylabel('PSEC-Net Trial Accuracy (%)', fontsize=12, fontweight='bold')
    ax.set_title('PSEC-Net: Per-Subject Trial-Quarantined Emotion Recognition on SEED-IV (300 DPI)', fontsize=13, fontweight='bold', pad=12)
    ax.legend(frameon=True, loc='upper left', fontsize=10)
    ax.grid(axis='y', linestyle='--', alpha=0.4)
    plt.tight_layout()
    bar_path = os.path.join(fig_dir, 'psec_per_subject_bar.png')
    ...
    ax.set(
        ...
        xlabel='PSEC-Net Super-Evidential Consensus Prediction'
    )
    ax.set_title(f'PSEC-Net: 1,080-Trial Zero-Leakage Confusion Matrix\n(Overall Trial Acc: {pop_trial_metrics["accuracy"]*100:.2f}%, F1: {pop_trial_metrics["f1"]:.4f})', fontsize=12, fontweight='bold', pad=12)
    ...
    plt.tight_layout()
    cm_path = os.path.join(fig_dir, 'psec_confusion_matrix.png')
    ...
    ax.set_title(f'PSEC-Net: Multiclass ROC Curves (1,080 Trials, Macro AUC: {pop_trial_metrics["auc"]:.4f})', fontsize=12, fontweight='bold', pad=12)
    ax.legend(loc="lower right", frameon=True, fontsize=10)
    ax.grid(True, linestyle='--', alpha=0.4)
    plt.tight_layout()
    roc_path = os.path.join(fig_dir, 'psec_roc_curves.png')
    plt.savefig(roc_path, dpi=300)
    plt.close()
    print(f"Generated publication figure: {roc_path}", flush=True)
    
    # Mirror figures to artifact directory
    artifact_fig_dir = r"C:\Users\Daksh's pc\.gemini\antigravity\brain\e5c12706-2777-497e-b3d6-0e26e7492dba\figures\psec_net"
    os.makedirs(artifact_fig_dir, exist_ok=True)
    for f_name in ['psec_per_subject_bar.png', 'psec_confusion_matrix.png', 'psec_roc_curves.png']:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="PSEC-Net SOTA Benchmark on SEED-IV")
    parser.add_argument('--device', type=str, default='cuda', choices=['cuda', 'cpu'], help="Compute device ('cuda' or 'cpu')")
    parser.add_argument('--dry_run', action='store_true', help="Execute single session verification on Subject 02 Session 1")
    parser.add_argument('--epochs', type=int, default=30, help="Number of training epochs per fold")
    args = parser.parse_args()
    
    run_psec_benchmark(args)
