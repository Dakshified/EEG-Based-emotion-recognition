"""
Top-K Climax Extraction & Quadratic Evidential Consensus Network (TopK-Quadratic-Net) SOTA Pipeline
===================================================================================================
Evaluates TopK-Quadratic-Net multimodal cortical-ocular alignment architecture across SEED-IV
...
Algorithmic Pipeline (TopK-Quadratic-Net):
1. Streamlined Late-Fusion Manifold (No Co-Attention Overfitting):
   ...
   - Evidential Dirichlet Head: Linear(192, 128) -> LayerNorm -> GELU -> Dropout(0.30) -> Linear(128, 4) -> Softplus() -> evidence e_w >= 0.
2. Top-K Climax Extraction:
   - Drop first 3s (onset) and last 2s (offset) per trial.
   - Rank candidate windows by high-frequency power E_w = mean(DE_beta + DE_gamma).
   - Retain strictly the Top K=5 highest-energy peak windows per trial.
3. Quadratic Evidential Consensus Aggregator:
   - Belief masses: b_w(c) = e_w(c) / S_w, Uncertainty: u_w = 4 / S_w.
   - Consensus Score:
     Score_trial(c) = sum_{w in Top-5} (1 - u_w) * (b_w(c))^2
...
def get_topk_climax_mask_for_trials(features_3d, trial_ids, k=5, onset_trim=3, offset_trim=2):
    """
    Top-K Climax Extraction:
    ...
    3. Rank candidate windows and retain strictly Top K=5 peak windows per trial.
    """
    ...
    for t in unique_trials:
        ...
        topk_idx = candidate_idx[np.argsort(energies)[-k_val:]]
        mask[topk_idx] = True
    return mask

class TopKQuadraticNet(nn.Module):
    """
    Top-K Quadratic Evidential Consensus Network (TopK-Quadratic-Net).
    Streamlined Late-Fusion Manifold (No Co-Attention Overfitting).
    """
    def __init__(self, eeg_dim=580, eye_dim=31, latent_eeg=128, latent_eye=64, num_classes=4, dropout=0.30):
        ...
        # Evidential Dirichlet Head: (128 + 64 = 192) -> 128 -> 4
        joint_dim = latent_eeg + latent_eye
...
def train_and_eval_single_session_topk(
    sub_id, sess_id, X_sess_3d, y_sess, trial_ids_sess, device, epochs=30, batch_size=32, n_splits=4, top_k=5, lr=1e-3, weight_decay=1e-4
):
    unique_trials = []
    trial_labels = []
    ...
    topk_climax_mask_all = get_topk_climax_mask_for_trials(X_sess_3d, trial_ids_sess, k=top_k, onset_trim=3, offset_trim=2)
    ...
    for fold, (train_trial_idx, test_trial_idx) in enumerate(skf.split(unique_trials, trial_labels)):
        ...
        train_climax_mask = train_frame_mask & topk_climax_mask_all
        ...
        test_climax_mask = topk_climax_mask_all[test_frame_mask]
        ...
        # TopK-Quadratic-Net Model
        set_seed(42 + fold)
        model = TopKQuadraticNet(eeg_dim=580, eye_dim=31, latent_eeg=128, latent_eye=64, num_classes=4, dropout=0.30).to(device)
        ...
        with torch.no_grad():
            with torch.amp.autocast('cuda', enabled=use_amp):
                test_evidence = model(test_eeg_tensor, test_eye_tensor)
        ...
        # Quadratic Evidential Consensus on Top-5 Climax Windows
        for t_val in test_trials_fold:
            ...
            if np.sum(k_climax) > 0:
                trial_b = test_belief_np[k_climax]      # (K, 4)
                trial_u = test_uncertainty_np[k_climax] # (K,)
            else:
                trial_b = test_belief_np[k_mask]
                trial_u = test_uncertainty_np[k_mask]
                
            # Score_trial(c) = sum_{w in Top-5} (1 - u_w) * (b_w(c))^2
            weights = (1.0 - trial_u)[:, None]          # (K, 1)
            quadratic_belief = np.square(trial_b)       # (K, 4)
            trial_scores = np.sum(weights * quadratic_belief, axis=0) # (4,)
...
def plot_topk_figures(all_trial_y_true, all_trial_y_pred, all_trial_y_prob, session_results, output_dir="figures/topk_consensus"):
    ...
    plt.title("TopK-Quadratic-Net Subject-Dependent Trial Consensus Accuracy per Subject (SEED-IV)", fontsize=14, fontweight='bold', pad=12)
    plt.ylim(0, 100)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "topk_per_subject_bar.png"), dpi=300)
    plt.close()
    
    # 2. 1,080-Trial Confusion Matrix
    ...
    plt.title(f"TopK-Quadratic-Net 1,080-Trial Confusion Matrix (Acc: {accuracy_score(all_trial_y_true, all_trial_y_pred)*100:.2f}%)", fontsize=13, fontweight='bold', pad=12)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "topk_confusion_matrix.png"), dpi=300)
    plt.close()
    ...
    plt.title('TopK-Quadratic-Net 1,080-Trial One-vs-Rest ROC Curves', fontsize=13, fontweight='bold', pad=12)
    plt.legend(loc="lower right", frameon=True, facecolor='white', framealpha=0.9, fontsize=11)
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "topk_roc_curves.png"), dpi=300)
    plt.close()
...
def run_dry_run(device, data_path="seed_iv_processed.npz"):
    print("\n==========================================================================")
    print(">>> RUNNING DRY RUN ON SUBJECT 15 SESSION 2 (TOPK-QUADRATIC-NET)")
    print("==========================================================================")
    ...
    # 1. Verify Top-5 Climax extraction
    top5_mask = get_topk_climax_mask_for_trials(X_sess, trial_ids_sess, k=5, onset_trim=3, offset_trim=2)
    print(f"[*] Top-5 Climax Windows Count: {np.sum(top5_mask)} (expected {len(np.unique(trial_ids_sess))*5} windows).")
    ...
    model = TopKQuadraticNet(eeg_dim=580, eye_dim=31, latent_eeg=128, latent_eye=64, num_classes=4, dropout=0.30).to(device)
    evidence = model(torch.tensor(eeg_580d).to(device), torch.tensor(eye_31d).to(device))
    
    print(f"[*] Model forward pass successful. Evidence shape: {evidence.shape}, Non-negative: {torch.all(evidence >= 0).item()}")
    
    # 3. Run full single-session Stratified 4-Fold CV
    start_t = time.time()
    res = train_and_eval_single_session_topk(
        sub_id=15, sess_id=2, X_sess_3d=X_sess, y_sess=y_sess, trial_ids_sess=trial_ids_sess,
        device=device, epochs=30, batch_size=32, n_splits=4, top_k=5, lr=1e-3, weight_decay=1e-4
    )
...
def main():
    parser = argparse.ArgumentParser(description="TopK-Quadratic-Net SOTA Pipeline")
    ...
    parser.add_argument("--figure_dir", type=str, default="figures/topk_consensus")
    ...
    plot_topk_figures(all_trial_y_true, all_trial_y_pred, all_trial_y_prob, all_session_results, output_dir=args.figure_dir)
    
    # Save JSON & CSV
    json_output = {
        'model_name': 'TopK-Quadratic-Net',
    ...
    json_path = os.path.join(args.output_dir, "topk_consensus_results.json")
    with open(json_path, 'w') as f:
        json.dump(json_output, f, indent=2, default=convert_numpy)
    print(f"[OK] Saved {json_path}")
    
    csv_path = os.path.join(args.output_dir, "topk_consensus_results.csv")
