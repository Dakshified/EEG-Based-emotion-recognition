...
def train_and_evaluate_fold(X_train_raw, y_train, d_train, X_val_raw, y_val, X_test_raw, y_test, n_domains, device, max_epochs=100, patience=15, batch_size=128):
    """
    Trains Inductive DANN with dynamic GRL lambda schedule, early stopping on validation emotion accuracy,
    and evaluates on test set. Validation and test domain labels are never used during training.
    """
    # Fit StandardScaler strictly on training samples
...
def run_cross_subject(features_flat, labels, subject_ids, device, figures_dir):
    """Cross-subject protocol: 5-fold Leave-3-Subjects-Out nested CV with 10 source domains."""
    print("=" * 80, flush=True)
    print("RUNNING INDUCTIVE DANN: Cross-Subject Protocol (Leave-3-Subjects-Out)", flush=True)
    print("=" * 80, flush=True)
    ...
    for fold, test_subs in enumerate(subject_folds):
        ...
        # Map 10 train subjects to domain IDs 0..9
        ...
        res = train_and_evaluate_fold(X_train, y_train, d_train, X_val, y_val, X_test, y_test, len(train_subs), device)
    ...
    np.savez_compressed(
        "oof_preds_dann_cross_subject.npz",
        y_true=y_true_all,
        y_pred=y_pred_all,
        y_prob=y_prob_all
    )
...
def run_subject_dependent(features_flat, labels, subject_ids, session_nums, trial_ids, device, figures_dir):
    """Subject-dependent protocol: pooled data, 5-fold nested CV grouped by trial with 15 subject domains."""
    print("=" * 80, flush=True)
    print("RUNNING INDUCTIVE DANN: Subject-Dependent Protocol (Pooled, 5-Fold Trial-Grouped CV)", flush=True)
    print("=" * 80, flush=True)
    ...
    for fold, (train_trial_idx, test_trial_idx) in enumerate(kf_outer.split(np.arange(num_trials))):
        ...
        y_train = labels[train_mask]
        s_train = subject_ids[train_mask]
        # In subject-dependent, subjects are 1..15 -> map to 0..14
        d_train = s_train - 1
        ...
        res = train_and_evaluate_fold(X_train, y_train, d_train, X_val, y_val, X_test, y_test, 15, device)
        fold_time = time.time() - t_fold_start
    ...
    prefix = "dann_subject_dependent"
    save_evaluation_plots(y_true_all, y_pred_all, y_prob_all, prefix, figures_dir)
    
    # Save out-of-fold sample predictions for verified downstream statistical testing
    np.savez_compressed(
        "oof_preds_dann_subject_dependent.npz",
        y_true=y_true_all,
        y_pred=y_pred_all,
        y_prob=y_prob_all
    )
...
def main():
    ...
    print("=" * 80, flush=True)
    print("INDUCTIVE-ONLY DANN BASELINE (GPU)", flush=True)
    ...
    res_cross = run_cross_subject(features_flat, labels, subject_ids, device, figures_dir)
    
    # 2. Run Subject-Dependent Protocol
    res_dep = run_subject_dependent(features_flat, labels, subject_ids, session_nums, trial_ids, device, figures_dir)
    ...
    print("\n" + "=" * 80, flush=True)
    print("FINAL SUMMARY: INDUCTIVE DANN RESULTS", flush=True)
    print("=" * 80, flush=True)
    ...
    print("=" * 80, flush=True)
    print(f"Total Inductive DANN baseline pipeline runtime: {(time.time() - t_start)/60:.2f} minutes.", flush=True)
