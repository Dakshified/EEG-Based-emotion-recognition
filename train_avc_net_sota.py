"""
Valence-Aware Climax & Margin-Gated Evidential Attention Network (AVC-Net) SOTA Pipeline
========================================================================================
Evaluates AVC-Net multimodal cortical-ocular alignment architecture across SEED-IV
...
Algorithmic Pipeline (AVC-Net):
1. Valence-Aware Climax Extraction:
   - High-frequency power: E_w = mean(DE_beta + DE_gamma)
   - Frontal Alpha Asymmetry: FAA_w = DE(Fp1, alpha) - DE(Fp2, alpha)
   - Joint Salience Score: S_w = 0.6 * Normalize(E_w) + 0.4 * Normalize(abs(FAA_w))
   - Retain strictly Top K=6 high-salience windows per trial.
2. Trial-Adaptive Local Normalization:
   - Apply intra-trial z-score normalization to isolate relative emotional fluctuations from tonic fatigue.
3. Streamlined Late-Fusion & Evidential Attention Head:
   - Cortical Stream: 580D -> Linear(580, 256) -> LayerNorm -> GELU -> Dropout(0.30) -> Linear(256, 128) -> LayerNorm -> GELU -> H_cortical.
   - Ocular Stream: 31D -> Linear(31, 64) -> LayerNorm -> GELU -> Dropout(0.30) -> Linear(64, 64) -> LayerNorm -> GELU -> H_ocular.
   - Joint Representation: z_w = [H_cortical, H_ocular] in R^192.
   - Evidential Dirichlet Head: Linear(192, 128) -> LayerNorm -> GELU -> Dropout(0.30) -> Linear(128, 4) -> Softplus() -> e_w >= 0.
   - Self-Attention Coherence Pooling: a_w = softmax(q^T tanh(W_a z_w)) over Top-6 climax windows.
4. Margin-Gated Quadratic Consensus Accumulator:
   - Score_trial(c) = sum_{w in Top-6} (1 - u_w) * a_w * (b_w(c))^2
...
def get_avc_salience_mask_for_trials(features_3d, trial_ids, k=6, onset_trim=3, offset_trim=2, w_energy=0.6, w_faa=0.4):
    """
    Valence-Aware Climax Extraction:
    1. Drop first onset_trim seconds (3s) and last offset_trim seconds (2s) per trial.
    2. Compute intra-trial high-frequency power E_w = mean(DE_{beta, w} + DE_{gamma, w}).
    3. Compute Frontal Alpha Asymmetry FAA_w = DE(Fp1, alpha) - DE(Fp2, alpha).
    4. Compute Min-Max normalized joint salience: S_w = w_energy * E_norm + w_faa * FAA_norm.
    5. Retain strictly Top K=6 highest-salience windows per trial.
    """
    ...
    for t in unique_trials:
        ...
        # 1. High-frequency energy across all 62 channels
        energies = np.mean(features_3d[candidate_idx, :, 3] + features_3d[candidate_idx, :, 4], axis=1) # (C_len,)
...
def apply_trial_adaptive_local_norm(feats_2d, trial_ids):
    """
    Trial-Adaptive Local Energy Normalization:
    Normalizes intra-trial feature scale by scalar trial RMS energy,
    preserving the essential neutral-referenced DC valence vector.
    """
    feats_norm = np.copy(feats_2d)
    unique_trials = np.unique(trial_ids)
    for t in unique_trials:
        idx = np.where(trial_ids == t)[0]
        if len(idx) > 1:
            rms_scale = float(np.sqrt(np.mean(np.square(feats_norm[idx])))) + 1e-4
            feats_norm[idx] = feats_norm[idx] / rms_scale
    return feats_norm

class AVCNet(nn.Module):
    """
    Valence-Aware Climax & Margin-Gated Evidential Attention Network (AVC-Net).
    ...
    def __init__(self, eeg_dim=580, eye_dim=31, latent_eeg=128, latent_eye=64, num_classes=4, dropout=0.30):
        ...
        # Cortical Stream Encoder: 580 -> 256 -> 128
        ...
        # Ocular Stream Encoder: 31 -> 64 -> 64
        ...
        # Self-Attention Coherence Pooling Layer
    ...
    def forward(self, x_eeg, x_eye):
        ...
        z = torch.cat([h_cortical, h_ocular], dim=-1) # (B, 192)
    ...
    def compute_attention_weights(self, z_seq):
        """
        z_seq: (K, 192) for K windows of a trial.
        returns: (K,) normalized attention weights summing to 1.
        """
        scores = self.attn_query(z_seq).squeeze(-1) # (K,)
...
def train_and_eval_single_session_avc(
    sub_id, sess_id, X_sess_3d, y_sess, trial_ids_sess, device, epochs=30, batch_size=32, n_splits=4, top_k=6, lr=1e-3, weight_decay=1e-4
):
    unique_trials = []
    trial_labels = []
    ...
    # 2. Valence-Aware Climax Mask (Top K=6)
    avc_mask_all = get_avc_salience_mask_for_trials(X_sess_3d, trial_ids_sess, k=top_k, onset_trim=3, offset_trim=2)
    ...
    for fold, (train_trial_idx, test_trial_idx) in enumerate(skf.split(unique_trials, trial_labels)):
        ...
        # HARD ZERO-LEAKAGE TRIAL QUARANTINE ASSERTION
        ...
        train_climax_mask = train_frame_mask & avc_mask_all
        ...
        train_t_ids_fold = trial_ids_sess[train_climax_mask]
        ...
        test_climax_mask = avc_mask_all[test_frame_mask]
        
        # BASELINE SUBTRACTION QUARANTINE (Calculated strictly from train neutral trials)
        ...
        # INDUCTIVE STANDARDIZATION ASSERTION (Fitted strictly on train trials)
        ...
        # Trial-Adaptive Local Normalization on scaled features
        X_eeg_train_scaled = apply_trial_adaptive_local_norm(X_eeg_train_scaled, train_t_ids_fold)
        X_eeg_test_scaled = apply_trial_adaptive_local_norm(X_eeg_test_scaled, test_t_ids_fold)
        X_eye_train_scaled = apply_trial_adaptive_local_norm(X_eye_train_scaled, train_t_ids_fold)
        X_eye_test_scaled = apply_trial_adaptive_local_norm(X_eye_test_scaled, test_t_ids_fold)
        ...
        # AVC-Net Model
        set_seed(42 + fold)
        model = AVCNet(eeg_dim=580, eye_dim=31, latent_eeg=128, latent_eye=64, num_classes=4, dropout=0.30).to(device)
        ...
        with torch.no_grad():
            with torch.amp.autocast('cuda', enabled=use_amp):
                ...
                test_uncertainty_np = test_uncertainty_tensor.cpu().numpy().squeeze(-1) # (N_test,)
                test_belief_np = test_belief_tensor.cpu().numpy()                       # (N_test, 4)
        ...
        # Self-Attention Coherence & Quadratic Evidential Consensus on Top-6 Climax Windows
        for t_val in test_trials_fold:
            ...
            # Compute Self-Attention Weights across the trial's climax windows
            with torch.no_grad():
                z_sub = test_z[trial_indices]            # (K, 192)
                attn_w = model.compute_attention_weights(z_sub).cpu().numpy() # (K,)
                
            # Score_trial(c) = sum_{w in Top-6} (1 - u_w) * attn_w * exp(M_w) * (b_w(c))^2
...
def plot_avc_figures(all_trial_y_true, all_trial_y_pred, all_trial_y_prob, session_results, output_dir="figures/avc_net"):
    ...
    plt.title("AVC-Net Subject-Dependent Trial Consensus Accuracy per Subject (SEED-IV)", fontsize=14, fontweight='bold', pad=12)
    plt.ylim(0, 100)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "avc_per_subject_bar.png"), dpi=300)
    plt.close()
    ...
    plt.title(f"AVC-Net Trial Confusion Matrix (Acc: {accuracy_score(all_trial_y_true, all_trial_y_pred)*100:.2f}%)", fontsize=13, fontweight='bold', pad=12)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "avc_confusion_matrix.png"), dpi=300)
    plt.close()
    ...
    plt.title('AVC-Net Multiclass One-vs-Rest ROC Curves', fontsize=13, fontweight='bold', pad=12)
    plt.legend(loc="lower right", frameon=True, facecolor='white', framealpha=0.9, fontsize=11)
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "avc_roc_curves.png"), dpi=300)
    plt.close()
...
def run_dry_run(device, data_path="seed_iv_processed.npz"):
    print("\n==========================================================================")
    print(">>> RUNNING DRY RUN ON SUBJECT 15 SESSION 2 (AVC-NET)")
    print("==========================================================================")
    ...
    mask = (sub_ids == 15) & (sess_ids == 2)
    X_sess = X[mask]
    y_sess = y[mask]
    trial_ids_sess = trial_ids[mask]
    
    print(f"[*] Loaded Subject 15 Session 2: {len(X_sess)} frames, {len(np.unique(trial_ids_sess))} trials.")
    
    # 1. Verify Valence-Aware Climax extraction
    avc_mask = get_avc_salience_mask_for_trials(X_sess, trial_ids_sess, k=6, onset_trim=3, offset_trim=2)
    print(f"[*] Top-6 Valence-Aware Climax Windows: {np.sum(avc_mask)} (expected {len(np.unique(trial_ids_sess))*6} windows).")
    
    # 2. Verify Tensor Forward & Shapes
    eeg_580d = extract_580d_cortical_features(X_sess[:16])
    eye_31d = extract_31d_ocular_features(X_sess[:16])
    
    model = AVCNet(eeg_dim=580, eye_dim=31, latent_eeg=128, latent_eye=64, num_classes=4, dropout=0.30).to(device)
    evidence, z = model(torch.tensor(eeg_580d).to(device), torch.tensor(eye_31d).to(device))
    attn_weights = model.compute_attention_weights(z[:6])
    
    print(f"[*] Model forward pass successful.")
    print(f"    - Evidence shape: {evidence.shape}, Non-negative: {torch.all(evidence >= 0).item()}")
    print(f"    - Latent z shape: {z.shape}")
    print(f"    - Attention weights shape: {attn_weights.shape}, Sum: {attn_weights.sum().item():.4f}")
    
    # 3. Run full single-session Stratified 4-Fold CV
    start_t = time.time()
    res = train_and_eval_single_session_avc(
        sub_id=15, sess_id=2, X_sess_3d=X_sess, y_sess=y_sess, trial_ids_sess=trial_ids_sess,
        device=device, epochs=30, batch_size=32, n_splits=4, top_k=6, lr=1e-3, weight_decay=1e-4
    )
...
def main():
    parser = argparse.ArgumentParser(description="AVC-Net SOTA Pipeline")
    ...
    parser.add_argument("--figure_dir", type=str, default="figures/avc_net")
    ...
    for sub in unique_subs:
        for sess in unique_sess:
            ...
            mask = (sub_ids == sub) & (sess_ids == sess)
            X_sess = X[mask]
            y_sess = y[mask]
            trial_ids_sess = trial_ids[mask]
            ...
            t0 = time.time()
            res = train_and_eval_single_session_avc(
                sub_id=sub, sess_id=sess, X_sess_3d=X_sess, y_sess=y_sess, trial_ids_sess=trial_ids_sess,
                device=device, epochs=args.epochs, batch_size=args.batch_size, n_splits=4, top_k=args.top_k,
    ...
    # Export Figures & Structured Results
    plot_avc_figures(all_trial_y_true, all_trial_y_pred, all_trial_y_prob, all_session_results, output_dir=args.figure_dir)
    ...
    json_path = os.path.join(args.output_dir, "avc_net_results.json")
    with open(json_path, 'w') as f:
        json.dump(json_output, f, indent=2, default=convert_numpy)
    print(f"[OK] Saved {json_path}")
    
    csv_path = os.path.join(args.output_dir, "avc_net_results.csv")
