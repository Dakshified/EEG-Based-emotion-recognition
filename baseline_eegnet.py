...
# =========================================================================
# COMPACT EEGNET ARCHITECTURE (1,484 Trainable Parameters)
# =========================================================================

class CompactEEGNet(nn.Module):
    def __init__(self, n_channels=62, n_bands=5, n_classes=4, F1=8, D=2, F2=16, dropout_rate=0.25):
        super().__init__()
        # 1. Band temporal / spectral conv
        self.conv1 = nn.Conv2d(1, F1, kernel_size=(1, 5), padding=(0, 2), bias=False)
        self.bn1 = nn.BatchNorm2d(F1)
        
        # 2. Spatial depthwise conv across all 62 electrodes
        ...
        # 3. Separable conv block (depthwise + pointwise)
        self.separable_depth = nn.Conv2d(F1 * D, F1 * D, kernel_size=(1, 3), padding=(0, 1), groups=F1 * D, bias=False)
...
def train_and_evaluate_fold(X_train_raw, y_train, X_val_raw, y_val, X_test_raw, y_test, device, ckpt_path=None, max_epochs=100, patience=15, batch_size=128):
    """Trains Compact EEGNet with early stopping on validation accuracy and evaluates on test set."""
    # Zero leakage: fit scaler strictly on training split
    scaler = StandardScaler()
    X_train_scaled = scaler.fit_transform(X_train_raw.reshape(len(X_train_raw), -1)).reshape(-1, 1, 62, 5)
    X_val_scaled = scaler.transform(X_val_raw.reshape(len(X_val_raw), -1)).reshape(-1, 1, 62, 5)
    X_test_scaled = scaler.transform(X_test_raw.reshape(len(X_test_raw), -1)).reshape(-1, 1, 62, 5)
    ...
    val_loader = DataLoader(
        TensorDataset(torch.tensor(X_val_scaled, dtype=torch.float32), torch.tensor(y_val, dtype=torch.long)),
    ...
    model = CompactEEGNet(n_channels=62, n_bands=5, n_classes=4, F1=8, D=2, F2=16, dropout_rate=0.25).to(device)
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        if epoch % 5 == 0 or epoch == 1 or mark == '*':
            print(f"    Epoch [{epoch:03d}/{max_epochs:03d}] ({t_epoch:.2f}s) | Train Loss: {train_loss:.4f} Acc: {train_acc:.4f} | Val Loss: {val_loss:.4f} Acc: {val_acc:.4f} {mark}", flush=True)
    ...
    if ckpt_path is not None:
        ...
        torch.save({'state_dict': best_weights, 'best_epoch': best_epoch, 'best_val_acc': best_val_acc}, ckpt_path)
    ...
    return {
        ...
        'y_pred': y_pred_arr,
        'y_prob': y_prob_arr,
        'y_val_true': np.array(val_targets),
...
def run_subject_dependent(features, labels, subject_ids, session_nums, trial_ids, device, figures_dir):
    """Subject-dependent protocol: pooled data, 5-fold nested CV grouped by trial."""
    print("=" * 80, flush=True)
    print("RUNNING COMPACT EEGNET: Subject-Dependent Protocol (Pooled, 5-Fold Trial-Grouped CV)", flush=True)
    print("=" * 80, flush=True)
    ...
    for fold, (train_trial_idx, test_trial_idx) in enumerate(kf_outer.split(np.arange(num_trials))):
        ...
        # Inner 80/20 train/val split on outer training trials
        ...
        ckpt_path = os.path.join("checkpoints", "eegnet", f"eegnet_subject_dependent_fold{fold+1}.pt")
        res = train_and_evaluate_fold(X_train, y_train, X_val, y_val, X_test, y_test, device, ckpt_path=ckpt_path)
    ...
    prefix = "eegnet_subject_dependent"
    ...
    np.savez_compressed(
        "oof_preds_eegnet_subject_dependent.npz",
        y_true=y_true_all,
        y_pred=y_pred_all,
        y_prob=y_prob_all
    )
...
def run_cross_subject(features, labels, subject_ids, device, figures_dir):
    """Cross-subject protocol: 5-fold Leave-3-Subjects-Out nested CV."""
    print("=" * 80, flush=True)
    print("RUNNING COMPACT EEGNET: Cross-Subject Protocol (Leave-3-Subjects-Out)", flush=True)
    print("=" * 80, flush=True)
    ...
    for fold, test_subs in enumerate(subject_folds):
        ...
        # Hold out 2 subjects from the 12 train pool subjects as inner validation set
        ...
        ckpt_path = os.path.join("checkpoints", "eegnet", f"eegnet_cross_subject_fold{fold+1}.pt")
        res = train_and_evaluate_fold(X_train, y_train, X_val, y_val, X_test, y_test, device, ckpt_path=ckpt_path)
        fold_time = time.time() - t_fold_start
    ...
    prefix = "eegnet_cross_subject"
    save_evaluation_plots(y_true_all, y_pred_all, y_prob_all, prefix, figures_dir)
    
    # Save out-of-fold sample predictions for verified downstream statistical testing
    np.savez_compressed(
        "oof_preds_eegnet_cross_subject.npz",
        y_true=y_true_all,
        y_pred=y_pred_all,
        y_prob=y_prob_all
    )
...
def main():
    ...
    print("=" * 80, flush=True)
    print("COMPACT EEGNET BASELINE (GPU)", flush=True)
    ...
    # 1. Run Subject-Dependent Protocol
    res_dep = run_subject_dependent(features, labels, subject_ids, session_nums, trial_ids, device, figures_dir)
    
    # 2. Run Cross-Subject Protocol
    res_cross = run_cross_subject(features, labels, subject_ids, device, figures_dir)
    ...
    json_path = "eegnet_baseline_results.json"
    with open(json_path, "w") as f:
        json.dump(all_results, f, indent=4)
    print(f"Saved structured JSON results to '{json_path}'", flush=True)
    
    csv_path = "eegnet_baseline_results.csv"
    ...
    print("\n" + "=" * 80, flush=True)
    print("FINAL SUMMARY: COMPACT EEGNET RESULTS", flush=True)
    print("=" * 80, flush=True)
    ...
    print("=" * 80, flush=True)
    print(f"Total Compact EEGNet baseline pipeline runtime: {(time.time() - t_start)/60:.2f} minutes.", flush=True)
