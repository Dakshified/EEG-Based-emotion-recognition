"""
Cortical-Ocular Manifold Alignment Network (COMA-Net) SOTA Pipeline
===================================================================
Evaluates COMA-Net dual-stream cortical-ocular alignment architecture across all 45 sessions
...
3. Intra-Trial Plateau Masking:
   - Trims initial 3 seconds (onset orienting) and trailing 2 seconds (offset habituation).
4. Evidential Dirichlet Trial Consensus:
   - Non-negative evidence e_w = Softplus(Linear(128, 4)), alpha_w = e_w + 1, u_w = 4 / S_w.
   - Dempster-Shafer consensus accumulation: E_trial = sum_{w in Plateau} (1 - u_w) * e_w.
   - Evidential Dirichlet Loss (digamma ACE + annealed KL divergence).
5. Zero-Leakage Assertions:
...
def extract_31d_ocular_features(features_3d):
    """
    Extracts 31D canonical ocular dynamics proxy features:
    - 12D Pupil dynamics proxy
    - 6D Saccadic dynamics proxy
    - 4D Fixation stability proxy
    - 4D Blink intensity proxy
    - 5D Cortical-Ocular coupling
    """
    N = len(features_3d)
...
def get_plateau_mask_for_trials(trial_ids, onset_trim=3, offset_trim=2):
    """
    Constructs boolean plateau mask per trial, trimming initial onset_trim and trailing offset_trim frames.
    """
...
class BidirectionalCoAttention(nn.Module):
    """
    Multi-Head Bidirectional Co-Attention between Cortical EEG and Ocular representations.
    """
...
class COMANet(nn.Module):
    """
    Cortical-Ocular Manifold Alignment Network.
    """
    ...
    def forward(self, x_eeg, x_eye):
        h_eeg_0 = self.eeg_encoder(x_eeg) # (B, 128)
        h_eye_0 = self.eye_encoder(x_eye) # (B, 128)
        
        h_eeg_att, h_eye_att = self.co_attention(h_eeg_0, h_eye_0) # (B, 128)
        
        # Gating factor gamma in [0, 1]
        concat_h = torch.cat([h_eeg_att, h_eye_att], dim=-1) # (B, 256)
        gamma = torch.sigmoid(self.gate_linear(concat_h))    # (B, 1)
        
        # Manifold Fused Latent State
        z = gamma * h_eeg_att + (1.0 - gamma) * h_eye_att   # (B, 128)
...
def train_and_eval_single_session_comanet(
    ...
    # 580D Cortical and 31D Ocular Features
    ...
    for fold, (train_trial_idx, test_trial_idx) in enumerate(skf.split(unique_trials, trial_labels)):
        ...
        # Apply plateau mask for training
        ...
        # COMA-Net Model
        model = COMANet(eeg_dim=580, eye_dim=31, latent_dim=128, num_classes=4).to(device)
...
def generate_publication_figures(
    ...
    rects2 = ax.bar(x + width/2, sub_trial_accs, width, label='COMA-Net Evidential Consensus Acc', color='#2ecc71', alpha=0.9, edgecolor='black', linewidth=0.8)
    ...
    ax.set_title(f'Cortical-Ocular Manifold Alignment Network (COMA-Net) Benchmark across 45 Sessions\nPer-Subject Frame vs. Evidential Trial Consensus Accuracy (Population Trial Acc = {pop_trial_acc:.2f}%)',
    ...
    plt.tight_layout()
    fig_path1 = os.path.join(output_dir, 'coma_net_per_subject_bar.png')
    ...
    ax.set_title(f'COMA-Net Evidential Trial Consensus Confusion Matrix (N = {len(pooled_y_trial)} Trials)\nAccuracy = {pop_trial_acc:.2f}% | Macro-F1 = {macro_f1:.4f}',
                 fontsize=12, fontweight='bold', pad=12)
    plt.tight_layout()
    fig_path2 = os.path.join(output_dir, 'coma_net_confusion_matrix.png')
    ...
    ax.set_title(f'COMA-Net Trial Evidential Consensus ROC Curves\nMacro ROC-AUC = {macro_auc:.4f} across {len(pooled_y_trial)} Trials', fontsize=13, fontweight='bold', pad=12)
    ax.grid(True, linestyle='--', alpha=0.5)
    ax.legend(loc='lower right', frameon=True, facecolor='white', framealpha=0.9, fontsize=11)
    plt.tight_layout()
    fig_path3 = os.path.join(output_dir, 'coma_net_roc_curves.png')
...
def main():
    parser = argparse.ArgumentParser(description="COMA-Net SOTA Benchmark on SEED-IV")
    ...
    print("=" * 80, flush=True)
    print("  Cortical-Ocular Manifold Alignment Network (COMA-Net)", flush=True)
    ...
    if args.dry_run:
        ...
        res = train_and_eval_single_session_comanet(
        ...
        print(f"[DRY RUN SUCCESS] Sub 15 Sess 2: Frame Acc = {res['frame_metrics']['accuracy']*100:.2f}%, Trial Evidential Acc = {res['trial_metrics']['accuracy']*100:.2f}% ({res['num_trials']} trials, mean gamma = {res['mean_gamma']:.4f})", flush=True)
        return
    ...
    print(f"Running COMA-Net benchmark across {len(unique_subs)} subjects (45 sessions total)...", flush=True)
    ...
    for sub_id in unique_subs:
        ...
        for sess_id in range(1, 4):
            ...
            print(f"  [Sub {sub_id:02d} Sess {sess_id}] Frame Acc: {f_acc:6.2f}% | Trial Evidential Acc: {t_acc:6.2f}% (gamma: {res['mean_gamma']:.3f}, {res['num_trials']} trials evaluated)", flush=True)
    ...
    print("=" * 80, flush=True)
    print("  FINAL COMA-NET POPULATION RESULTS (1,080 Trials, 37,575 Frames)", flush=True)
    print("=" * 80, flush=True)
    ...
    print(f"  Population Mean Gate : gamma = {pop_gamma:.4f} ({pop_gamma*100:.1f}% Cortical / {(1-pop_gamma)*100:.1f}% Ocular)", flush=True)
    print("=" * 80, flush=True)
    
    # Save results to JSON
    json_output = {
        'benchmark_name': 'Cortical-Ocular Manifold Alignment Network (COMA-Net)',
    ...
    json_path = 'coma_net_results.json'
    with open(json_path, 'w') as f:
        json.dump(json_output, f, indent=2)
    print(f"[Export] Saved structured benchmark results to {json_path}", flush=True)
    
    # Save CSV
    csv_path = 'coma_net_results.csv'
    with open(csv_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([
            'subject_id', 'sessions_evaluated', 'num_trials', 'num_frames', 'mean_gamma',
        ...
        for s in subject_summaries:
            writer.writerow([
                f"Sub {s['sub_id']:02d}", s['num_sessions'], s['num_trials'], s['num_frames'], f"{s['mean_gamma']:.4f}",
        ...
        writer.writerow([
            'Population (15 Subjects)', len(all_session_results), len(pooled_y_trial), len(pooled_y_frame), f"{pop_gamma:.4f}",
    ...
    output_dir = os.path.join('figures', 'coma_net')
    artifact_dir = os.path.join("C:\\Users\\Daksh's pc\\.gemini\\antigravity\\brain\\e5c12706-2777-497e-b3d6-0e26e7492dba", 'figures', 'coma_net')
