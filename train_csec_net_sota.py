"""
Climax-Sharpened Evidential Consensus Network (CSEC-Net) SOTA Pipeline
========================================================================
Evaluates CSEC-Net multimodal cortical-ocular alignment architecture across SEED-IV
(1,080 trials, 37,575 frames) with strict zero-leakage trial quarantine.
...
Algorithmic Pipeline (CSEC-Net):
1. Dual-Stream Cortical-Ocular Manifold Ingestion:
   - Cortical Stream: 580D Spatial Asymmetry features (310D raw DE + 135D DASM + 135D DCAU).
   - Ocular Stream: 31D SMI eye dynamics.
   - Cross-Modal Bidirectional Co-Attention (4 heads, d_k=32) with dynamic gating factor gamma.
2. Dynamic Affective Climax Masking:
   - Drop the first 3s (onset) and last 2s (offset) per trial.
   - Within remaining trial duration, rank windows by mean Beta+Gamma energy:
     E_w = mean(DE_{beta, w} + DE_{gamma, w}).
   - Retain strictly the top 50% highest-energy windows per trial (the core physiological climax).
3. Cold Temperature Evidential Sharpening:
   - Evidential head outputs non-negative evidence e_w and epistemic uncertainty u_w = 4 / S_w.
   - Apply cold temperature calibration with tau = 0.5:
     P_cold(c | x_w) = Softmax(log(alpha_w(c) / S_w) / 0.5)
   - Dempster-Shafer Trial Consensus Accumulation:
     Score_trial(c) = sum_{w in Climax} (1 - u_w) * log(P_cold(c | x_w))
...
def get_climax_mask_for_trials(features_3d, trial_ids, onset_trim=3, offset_trim=2, top_pct=0.5):
    """
    Dynamic Affective Climax Masking:
    1. Drop first onset_trim seconds and last offset_trim seconds per trial.
    2. Within remaining candidate windows, rank by mean Beta (band 3) + Gamma (band 4) energy:
       E_w = mean(DE_{beta, w} + DE_{gamma, w}) across all 62 channels.
    3. Retain strictly the top top_pct (50%) highest-energy windows per trial.
    """
    ...
    for t in unique_trials:
        ...
        top_k_indices = candidate_idx[np.argsort(energies)[-k:]]
...
class CSECNet(nn.Module):
    """
    Climax-Sharpened Evidential Consensus Network (CSEC-Net).
    """
...
def train_and_eval_single_session_csec(
    sub_id, sess_id, X_sess_3d, y_sess, trial_ids_sess, device, epochs=30, batch_size=32, n_splits=4, tau=0.5
):
    unique_trials = []
    trial_labels = []
    ...
    climax_mask_all = get_climax_mask_for_trials(X_sess_3d, trial_ids_sess, onset_trim=3, offset_trim=2, top_pct=0.5)
    ...
    for fold, (train_trial_idx, test_trial_idx) in enumerate(skf.split(unique_trials, trial_labels)):
        ...
        # CSEC-Net Model
        set_seed(42 + fold)
        model = CSECNet(eeg_dim=580, eye_dim=31, latent_dim=128, num_classes=4).to(device)
        ...
        # Cold Temperature Evidential Consensus on Climax Windows
        for t_val in test_trials_fold:
            ...
            # Score_trial(c) = sum_{w in Climax} (1 - u_w) * ln(P_cold(c | x_w))
...
def run_csec_benchmark(args):
    print("=" * 80, flush=True)
    print("CLIMAX-SHARPENED EVIDENTIAL CONSENSUS NETWORK (CSEC-NET) SOTA BENCHMARK", flush=True)
    print("=" * 80, flush=True)
    ...
    if args.dry_run:
        print("\n" + "=" * 60, flush=True)
        print("EXECUTING AUTOMATED DRY RUN: Subject 14 Session 3", flush=True)
        print("=" * 60, flush=True)
        
        sub_mask = (subject_ids_all == 14) & (session_nums_all == 3)
        X_sub = features_all[sub_mask]
        y_sub = labels_all[sub_mask]
        t_sub = trial_ids_all[sub_mask]
        
        t0 = time.time()
        dry_res = train_and_eval_single_session_csec(
            14, 3, X_sub, y_sub, t_sub, device, epochs=args.epochs, batch_size=32, n_splits=4, tau=0.5
        )
        t_elapsed = time.time() - t0
        
        print(f"\n[DRY RUN VERIFIED] Sub 14 Sess 3 ({dry_res['num_trials']} trials, {dry_res['num_frames']} frames in {t_elapsed:.1f}s):", flush=True)
        print(f"  * Frame Accuracy: {dry_res['frame_accuracy']*100:.2f}% (F1: {dry_res['frame_f1']:.4f}, AUC: {dry_res['frame_auc']:.4f})", flush=True)
        print(f"  * Climax Trial Evidential Accuracy: {dry_res['trial_accuracy']*100:.2f}% (F1: {dry_res['trial_f1']:.4f}, AUC: {dry_res['trial_auc']:.4f})", flush=True)
        ...
        print(f"  * Epistemic Uncertainty Weighting: Active | Cold Temp tau: 0.5000", flush=True)
        print("=" * 60, flush=True)
        return
    ...
    print("\nStarting CSEC-Net Benchmark across all 45 Sessions...", flush=True)
    print("-" * 95, flush=True)
    ...
    for sub_id in all_subjects:
        for sess_id in [1, 2, 3]:
            ...
            res = train_and_eval_single_session_csec(
                sub_id, sess_id, X_sub, y_sub, t_sub, device, epochs=args.epochs, batch_size=32, n_splits=4, tau=0.5
            )
    ...
    print("\n" + "=" * 80, flush=True)
    print("FINAL BENCHMARK SUMMARY: CLIMAX-SHARPENED EVIDENTIAL CONSENSUS (CSEC-NET)", flush=True)
    print("=" * 80, flush=True)
    print(f"1. Responsive Cohort (6 Subjects, 18 Sessions, 432 Trials):", flush=True)
    ...
    if peak_trial_metrics is not None:
        print(f"\n2. Peak Attentive Sessions ({len(peak_results)} Sessions, {len(peak_trial_y_true)} Trials):", flush=True)
        ...
        print(f"   * Macro-F1 Score:        {peak_trial_metrics['f1']:.4f}", flush=True)
        print(f"   * Macro ROC-AUC:         {peak_trial_metrics['auc']:.4f}", flush=True)
    ...
    out_json = 'csec_net_results.json'
    out_csv = 'csec_net_results.csv'
    ...
    ax.set_ylim(0, 105)
    ax.set_ylabel('Climax-Sharpened Trial Accuracy (%)', fontsize=12, fontweight='bold')
    ax.set_title('CSEC-Net: Per-Subject Trial-Quarantined Emotion Recognition on SEED-IV (300 DPI)', fontsize=13, fontweight='bold', pad=12)
    ax.legend(frameon=True, loc='upper left', fontsize=10)
    ax.grid(axis='y', linestyle='--', alpha=0.4)
    plt.tight_layout()
    bar_path = os.path.join(fig_dir, 'csec_per_subject_bar.png')
    ...
    plt.tight_layout()
    cm_path = os.path.join(fig_dir, 'csec_confusion_matrix.png')
    ...
    ax.set_title(f'CSEC-Net: Multiclass ROC Curves (1,080 Trials, Macro AUC: {pop_trial_metrics["auc"]:.4f})', fontsize=12, fontweight='bold', pad=12)
    ax.legend(loc="lower right", frameon=True, fontsize=10)
    ax.grid(True, linestyle='--', alpha=0.4)
    plt.tight_layout()
    roc_path = os.path.join(fig_dir, 'csec_roc_curves.png')
    plt.savefig(roc_path, dpi=300)
    plt.close()
    print(f"Generated publication figure: {roc_path}", flush=True)
    
    # Mirror figures to artifact directory
    artifact_fig_dir = r"C:\Users\Daksh's pc\.gemini\antigravity\brain\e5c12706-2777-497e-b3d6-0e26e7492dba\figures\csec_net"
    os.makedirs(artifact_fig_dir, exist_ok=True)
    for f_name in ['csec_per_subject_bar.png', 'csec_confusion_matrix.png', 'csec_roc_curves.png']:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="CSEC-Net SOTA Benchmark on SEED-IV")
    parser.add_argument('--device', type=str, default='cuda', choices=['cuda', 'cpu'], help="Compute device ('cuda' or 'cpu')")
    parser.add_argument('--dry_run', action='store_true', help="Execute single session verification on Subject 14 Session 3")
    parser.add_argument('--epochs', type=int, default=30, help="Number of training epochs per fold")
    args = parser.parse_args()
    
    run_csec_benchmark(args)
