...
### Baseline Experiments
Built a DGCNN baseline, with a learnable adjacency matrix initialized from real electrode geometry via `channel_62_pos.locs`, using the individual per-subject protocol and the same trial-level splitting established on 6 August.

### Outcome
A clear accuracy pattern emerged across baselines: **63.04% (SVM) / 59.12% (CNN-LSTM) / 47.23% (DGCNN)**.

### Issue
DGCNN accuracy was noticeably lower than the simpler baselines.

### Root Cause
Diagnosed as parameter-count-driven overfitting under small per-subject data — confirmed via train/val curves showing training accuracy up to 100% against validation accuracy near chance level.

### Resolution
Confirmed this was expected model behavior under the given data constraints, not a bug; no code changes required.

---

### Baseline Experiments
Built a plain multi-band Transformer baseline: 5 band-tokens, custom self-attention implemented to allow extraction of attention weights, evaluated under the same individual per-subject protocol.

### Outcome
- Subject-dependent accuracy: **63.05%** (matches the SVM baseline).
- Cross-subject accuracy: **33.75%**.

✅ Milestone Achieved — 4-baseline comparison table complete (SVM, CNN-LSTM, DGCNN, Transformer), satisfying SCI-level ablation rigor requirements.

---

### Planning
Clarified the project's novelty scope and evaluation plan in detail.

- Corrected a misunderstanding around "noise removal": reframed as noise/artifact **robustness**, achieved via channel-masking training, not data cleaning.
- Finalized the 6-model, two-round testing plan (clean test + shared corrupted test).

### Outcome
✅ Milestone Achieved — Novelty list finalized:

1. Leakage-free evaluation protocol
2. GAT + cross-band Transformer + KAN architecture
3. Faithfulness-verified explainability
4. Calibrated uncertainty / abstention
5. Noise-robust training with shared robustness evaluation across all 6 models

### Notes
> 🚧 Next Steps: Begin implementation of the GAT + cross-band Transformer + KAN architecture and set up the channel-masking noise-robustness training pipeline.
